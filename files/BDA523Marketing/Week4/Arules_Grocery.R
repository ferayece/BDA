...
# Groceries datasi transaction olarak tutuluyor, Apriori'ye transaction tipinde input vermek gerekiyor.
# Datami transaction'a nasil çeviririm?
# df <- read.csv(<file>)
# tr_df <- as(df,"transactions")    #transaction'a çevirmis olduk. 
...
inspect(head(Groceries))   #datayi incelemek için.

?Groceries
#dataframe to tran --> as.transacton)
...
#pdf: pdf açiyor bu bir device oluyor R için; dev.off() diyerek device la isin bitince kapatmak lazim.
#ggplot 20 grafik çikarcaksin; bu sekilde pdf'e atabilirsin.
...
# first.rules bir ruleset oluyor; support ve confidence'a göre olusturulmus bir ruleset.
...
#first.rules
#inspect(head(first.rules)) ruleseti incele.
# inspect(first.rules)  #tüm kural setini döker.
...
#df <- as(second.rules,"data.frame")
#View(df)
#çiktisindaki support(A,B) kombinasyonun support'una bakiyor. 
# supportu yüksek olmasini hayal ediyoruz ama mümkün degili.
# support dediginizde bu iki ürünün yan yana olmasina ne kadar güvenebiliriz? Transactionlarin kaçinda beraberler?
#confidence: iki ürün birlikte geçiyor, bunu alana bunu alani yüzde kaç oranla güvenerek önerebilirim?
# hep lifte bak; confidcen baskin ürünleri getirip seni yaniltabilir. (maydonoz dererotu confidence'i daha yüksektir ama öneremezsin dereotu, manasiz)
...
dd <- subset(second.rules,subset = lhs %in% groceries)
